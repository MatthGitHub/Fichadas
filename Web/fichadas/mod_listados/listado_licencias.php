...
if(($_POST['txtFechaDesde'])&&($_POST['txtFechaHasta'])&&($_POST['desde'])&&($_POST['hasta'])){

  $desde = $_POST['txtFechaDesde'];
  $hasta = $_POST['txtFechaHasta'];
  $DesdeLegajo = $_POST['desde'];
  $HastaLegajo = $_POST['hasta'];
  ...
  $sql = "SELECT e.legajo , e.apellido , e.nombre , CONVERT(VARCHAR(12),a.desdefecha,3) as desdefecha ,
          ...
          AND  a.desdefecha   BETWEEN  '$desde'
          AND  '$hasta'
...
if( $stmt === false ) {
  ...
  header("Location: listado_licencias.php?errordat");
  exit();
}
  ...
  <head>
   ...
   <body>
    <div class="container">
      ...
      <div class="jumbotron">
	  <h4 class="text-center bg-info">Listado licencias</h4>
      <form action="listado_licencias_print.php" method="post" target="_blank">
        ...
        <input name="desde" type="hidden" id="desde" value="<?php echo $DesdeLegajo; ?>">
        <input name="hasta" type="hidden" id="hasta" value="<?php echo $HastaLegajo; ?>">
        <label>
